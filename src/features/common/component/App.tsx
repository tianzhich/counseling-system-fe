...
class App extends React.Component<IAppProps, IAppState> {
  ...
  messageInterval: NodeJS.Timeout // 消息提醒
  ...
  handleLeaveMessage = (payload: any) => {
    this.messageModalRef.current.openModal(
      payload.receiverId,
  ...
  componentDidUpdate(prevProps: IAppProps) {
    ...
    if (!prevProps.isAuth && this.props.isAuth) {
      this.initRequest()
      this.messageInterval = setInterval(this.loadNotifications, MsgIntervalTime)
    }
  }
    ...
    return (
      <div className="pcs-app">
        <Layout>
          ...
          <Footer>Zhi Psychological Counseling System ©2018 Created by Tian Zhi</Footer>
        </Layout>
        <SignModal
          type={this.state.signModal}
          onChangeModal={this.handleChangeModal}
          ref={this.signModalRef}
        />
...
const mapState = (state: IStore) => ({
  ...
  // 数据
  notifications:
    state['query/notifications'].response && state['query/notifications'].response.data
      ? state['query/notifications'].response.data
      : [],
  messages:
    state['query/messages'].response && state['query/messages'].response.data
      ? state['query/messages'].response.data
      : [],
  onload:
    state['query/messages'].status === 'loading' ||
    state['query/notifications'].status === 'loading'
})
