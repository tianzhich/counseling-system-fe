    ...
    const userActionMap: ActionMap = {
      ...
      wait_counseling: (
        <React.Fragment>
          ...
          <Button type="primary" onClick={this.handleCompleteCounseling}>
            咨询已完成
          </Button>
        </React.Fragment>
      ),
      ...
      data.status === 'cancel' ? (
        <div className="section">
          ...
          <div className="content">
            {data.cancelReason1 ? (
              <div>
                <span>咨询者取消理由</span> {data.cancelReason1}
              </div>
            ) : null}
            {data.cancelReason2 ? (
              <div>
                <span>咨询师取消理由</span> {data.cancelReason2}
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="main">
          <div className="section">
            ...
            <div className="content">
              ...
              <div>
                <span>性别</span> {data.gender === 1 ? '男' : '女'}
              </div>
              <div>
                <span>年龄</span> {data.age}
              </div>
              <div>
                <span>电话</span> {data.phone}
              </div>
            </div>
          </div>
          <div className="section">
            ...
            <div className="content">
              <div>
                <span>姓名</span> {data.contactName}
              </div>
              <div>
                <span>关系</span> {data.contactRel}
              </div>
              <div>
                <span>电话</span> {data.contactPhone}
              </div>
            </div>
          </div>
          <div className="section">
            ...
            <div className="content">
              <div>
                <span>咨询方式</span> {CounselingMethodMap[data.method]}
              </div>
              <div>
                <span>咨询次数</span> {data.times}次
              </div>
              <div>
                <span>咨询时间</span> {data.startTime ? data.startTime : '暂无'}
              </div>
              <div>
                <span>咨询地点</span> {data.location ? data.location : '暂无'}
              </div>
              <div>
                <span>咨询描述</span>
                <p>{data.desc}</p>
